...
class TestBuildFeatureVector:
    def test_output_shape(self):
        X = build_feature_vector(_row(), _row(), _row(), _row(), _MEDIAN)
    ...
    def test_hour_sin_cos_at_midnight(self):
        X = build_feature_vector(_row(hour=0), _row(hour=0), _row(hour=0), _row(hour=0), _MEDIAN)
    ...
    def test_hour_sin_cos_at_noon(self):
        X = build_feature_vector(_row(hour=12), _row(hour=12), _row(hour=12), _row(hour=12), _MEDIAN)
        sin_idx = FEATURES.index("hour_sin")
        cos_idx = FEATURES.index("hour_cos")
    ...
    def test_none_value_falls_back_to_median(self):
        row = _row()
        row["co"] = None
        X = build_feature_vector(_row(), _row(), _row(), row, _MEDIAN)
        assert not np.any(np.isnan(X))
...
class TestPredictSingle:
    ...
    def test_predicted_aqi_in_range(self, model_median):
        ...
        X = build_feature_vector(_row(), _row(), _row(), _row(), median)
...
class TestBatchPredict:
    ...
    def test_too_few_rows_returns_empty(self, model_median):
        ...
        assert batch_predict(model, median, [_row(), _row(), _row()]) == []
        assert batch_predict(model, median, [_row(), _row(), _row(), _row()]) == []
    ...
    def test_output_length(self, model_median):
        ...
        # New formula: len(rows) - 4 (3-hour lookback + 1 ground-truth row)
    ...
    def test_actual_matches_next_row_aqi(self, model_median):
        """actual at position idx should equal aqi of rows[idx+4] (T+1 from the T-3..T window)."""
        model, median = model_median
        rows = [_row(aqi=(i % 5) + 1) for i in range(6)]
        results = batch_predict(model, median, rows)
        for idx, r in enumerate(results):
            expected_actual = ((idx + 4) % 5) + 1
