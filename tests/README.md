...
- 100 synthetic rows, 5 AQI classes, 19 features
...
|------------|----------------|
| TestBuildFeatureVector | Output shape is (19,), dtype is float32, no NaN in output, aqi_delta_1h and pm10_delta_1h are computed correctly, hour_sin and hour_cos are correct at midnight and noon, month_sin and month_cos are correct, pm25_ratio is non-negative, None and NaN inputs fall back to the median value |
...
| Test class | What it covers |
|------------|----------------|
| TestHealth | Returns HTTP 200, all expected schema keys are present, model_loaded is True, n_features is 19 |
